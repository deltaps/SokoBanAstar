...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
#------------------------ Ce module a pour but, à l'aide d'un algorithme a étoile, de trouver le chemin le plus court que le cookie doit empreinter pour atteindre son panier -------------------------------#
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
def a_etoile_cookie(dep,arr,colision):
    ...
    while len(openlist) > 0 :
        ...
        # Si on est arrivé au noeud de fin, alors il suffit simplement de retrouver le chemin inverse en remontant
        # le parent du parent du parent... etc. et nous avons le chemin le plus court :
...
# trouver les possibilités de déplacement
        ...
        for i in ([[50,0],[0,50],[0,-50],[-50,0]]) : # Les possibilités de déplacement pour notre jeu sont soit au dessus, en dessous, à gauche ou à droite.
             # Contrairement à l'algoAPrince, ici nous vérifions si la case opposé au déplacement est libre aussi car si elle n'est pas libre, le Prince ne peut pas s'y rendre et donc ne peut pas pousser le cookie.
            if [currentnode.position[0] - i[0] ,currentnode.position[1] - i[1]] not in colision :
